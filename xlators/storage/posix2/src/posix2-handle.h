...
#define POSIX_NAME_LOOKUP(this, pgfid, name, sbuf, parent, op_ret, op_errno) \
        do {                                                            \
        ...
        iatt_from_stat (&sbuf, &lstatbuf);                              \
        ret = posix_fill_gfid_from_path (this, file_path, &sbuf);       \
        if (ret == -1) {                                                \
                op_errno = errno;                                       \
                break;                                                  \
        }                                                               \
        posix_fill_ino_from_gfid (this, &sbuf);                         \
        op_ret = 0;                                                     \
        if (parent)                                                     \
...
/* the following exists as a macro solely to leverage alloca */
#define POSIX_INODE_LOOKUP(this, gfid, sbuf, fpath, op_ret, op_errno)   \
        do {                                                            \
        struct stat lstatbuf    = {0, };                                \
        char       *file_path   = NULL;                                 \
        ...
        iatt_from_stat (&sbuf, &lstatbuf);                              \
        gf_uuid_copy (sbuf.ia_gfid, gfid);                              \
        posix_fill_ino_from_gfid (this, &sbuf);                         \
        op_ret = 0;                                                     \
        if (fpath)                                                      \
                fpath = file_path;                                      \
