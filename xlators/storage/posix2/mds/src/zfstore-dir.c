...
static int32_t
zfstore_open_and_save (xlator_t *this, fd_t *fd,
                       char *entry, int32_t flags, mode_t mode)
{
        int openfd = -1;
        int32_t ret = 0;

        openfd = open (entry, flags, mode);
        if (openfd < 0)
                goto error_return;
                    ...
                    char *export, uuid_t gfid, fd_t *fd, int32_t flags)
{
        ...
        ret = zfstore_open_and_save (this, fd, entry, flags, 0);
        if (ret)
                goto error_return;
        return 0;
                      ...
                      mode_t mode, dict_t *xdata, struct iatt *stbuf)
{
        ...
        ret = zfstore_open_and_save (this, fd, entry, flags, mode);
        if (ret)
                goto purge_entry;
              ...
              loc_t *loc, int32_t flags, fd_t *fd, dict_t *xdata)
{
        ...
        zf = posix2_get_store (this);
        if (gf_uuid_is_null (loc->gfid)) {
                gf_msg (this->name, GF_LOG_ERROR, 0, POSIX2_MSG_INODE_NULL_GFID,
                       "null gfid for path %s", (loc)->path);
        }

        ret = zfstore_open_inode (this, zf->exportdir, loc->gfid, fd, flags);
        if (ret <= 0)
                goto unwind_err;
