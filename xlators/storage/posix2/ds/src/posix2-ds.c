...
posix2_ds_init (xlator_t *this)
{
        ...
        ds = GF_CALLOC (1, sizeof (*ds), gf_posix2_mt_private_t);
        if (!ds)
                goto error_return;
            ...
            loc_t *loc, int32_t flags, fd_t *fd, dict_t *xdata)
{
        ...
        op_ret = posix2_save_openfd (this, fd, _fd, flags);
        if (op_ret) {
                op_errno = errno;
...
_page_aligned_alloc (size_t size, char **aligned_buf)
{
        ...
        alloc_buf = GF_CALLOC (1, (size + ALIGN_SIZE), gf_posix2_mt_char);
        if (!alloc_buf)
                goto out;
        /* page aligned buffer */
              ...
              uint32_t flags, struct iobref *iobref, dict_t *xdata)
{
        ...
        */
        ...
        ret = posix2_fdstat (this, _fd, fd->inode->gfid, &postop);
        if (ret == -1) {
                op_ret = -1;
                op_errno = errno;
                gf_msg (this->name, GF_LOG_ERROR, errno,
